...
function showResults({result:r,ensemble,caseResults}){
 ...
 $('geometry-note').textContent=`Start: about ${result.startDistance.toFixed(1)} km from VNO. Coverage: Lithuania and the parts of Latvia and Poland inside the study box. Country boundaries are generalized; the 100 km margin uses the study projection. LT airspace: 11 June 2026 plus SUP 14/26; LV / PL: 3 September 2026. Live NOTAMs and activation are not loaded. Flight levels use a standard-pressure height approximation; terrain and AGL conversions are not modeled.`;
...
$('region-count').textContent=`${REGION.airspaces.length} published areas · LT / LV / PL`;
